...
int main() {
    ...
    float titleTime = 5000.0,characterBallPause = 400.0,enemyPause = 6000.0,enemyBallPause = 2500.0;
    ...
    while(window.isOpen()){
        ...
        for(int i=0;i<enemies.size();i++){
            if(enemies[i].getPosition().x<=0){
                ...
                if(++dragonsMissed>2){
                    cout<<"Game over. Too many missed dragons"<<endl;
                    return 0;
                }
            }
        }
            ...
            for(int o=0;o<enemyBalls.size();o++){
                if(isCharacterShooted(enemyBalls[o],character)){
                    ...
                    if(stoi(characterHp)<=0){
                        cout<<"Game over. Character destroyed"<<endl;
                        return 0;
                    }
                }
            }
